    ...
    switch (cmd) {
      ...
      case "bcco":
        var ee = $('.bid' + data.bid + '  .bccos');
		  $('<div style="border: 1px solid;float: right;width: 100%;padding: 2px;margin-bottom: -1px;" class="fl"><div class="fl" style="width: 87%;text-align: right;" c><span class="fr" style="width: 100%;">'+data.topic+'</span><span class="fl" style="color: #7e7c7c;width: 100%">'+data.pccus+'</span></div><img class="fr" src="'+data.pic+'" style="width: 30px;"></div>').prependTo(ee);
...
function updateu(id, uuu) {
  ...
  if (u.co == "--" || u.co == null || u.co == 'A1' || u.co == 'A2' || u.co == 'EU') {
    ...
    uh.find(".co").attr("src", "flag/" + u.co.toLowerCase() + ".png")
  }
  if (ismuted(u)) {
...
function AddMsg(wid, data) {
  ...
  if (data.bid != null) {
    ...
    if (power.delbc || data.lid == getuser(myid).lid) {
      msg.append('<a onclick="send(\'delbc\',{bid:\'' + data.bid + '\'})" style="margin-top:-20px;padding:4px;" class="btn minix btn-primary fa fa-times fa">&nbsp;</a>');
    }
    msg.append('<a onclick="send(\'likebc\',{bid:\'' + data.bid + '\'})" style="margin-top:-20px;padding:4px;" class="btn minix btn-primary fa fa-heart fr">&nbsp;</a>')
    msg.append('<a onclick="console.log($(this).parent().children(\'.sco\').toggle())" style="margin-top:-20px;padding:4px;" class="btn minix btn-primary fa fa-comment fr">&nbsp; </a>')
    msg.append('<div class="sco" style="display: none;width: 100%;"><input class="fl"   type="text" ><button onclick="send(\'sco\',{bid:\'' + data.bid + '\',c:$(this).parent().children(\'input\').val()});$(this).parent().toggle()" style=" margin-top:2px;margin-left:2px;" class="fa fa-send sndbc fl btn btn-primary">إرسال</button><div>')
